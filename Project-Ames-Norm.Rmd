...
```
----------------------
IMPORTANT:Explore Response Variable - ADD in visuals for FINAL REPORT
...
#To look at the distribution we look at the measure skewness, as a symmetrical dataset will have a skew of 0. To combat skewness and keep in range -1 to 1 going to log the numeric variables an absolute skew 
...
#for machine learning going to need to complete turning all to numeric. One hot encode is for categorical variables with no ordinality will be represented by 1's and 0's to show if exist. Going to try to use model.matrix function. 
...
#Want to look at only sales that are normal not abnormal (Sale.Condition) and of homes that have a basement!! (Bsmt.Cond is NA or now none means no basement.)...

Want to look at your typical sale of a home with a basement and garage!!
  
table(ames$Sale.Condition)
#2413 are Normal
table(ames$Bsmt.Cond)
#80 homes don't have a basement
table(ames$Garage.Cond)
#159 homes have no Garage
