---
title: "CAPSTONE - Housing Price Prediciton Project - EMENY""
...
#Some didnt use but played around with in kernels.
...
e.	Descriptive Statistics – summary, overview, explanation of each variable.
...
ames123 <- read.csv("~/Desktop/ames.csv", header = TRUE, sep = ',', stringsAsFactors = F) 

ames123$Order <- NULL
ames123$PID <- NULL

str(ames123)
NAcolumns <- names(which(sapply(ames123, anyNA)))
NAcolumns
...
NEED TO RUN THIS TWICE TO REMOVE NA"S: Basement Variables (Run 3 times may have to double run to make sure all NA's taken care of) (added 3 times so works for rmarkdown how intended)
```{r}
...
!!!!!!Re-Run after running bathroom variables 3 times to eliminate all Na's (13)...also have to re-run garage quality, finish and condition to make sure NA's taken care of. (4).!!!!!!!!1
```{r}
...
scatter.hist(ames$Total.Sq.Feet, ames$SalePrice)
...
scatter.hist(ames$Total.Bathrooms, ames$SalePrice)
...
rmse(test$SalePrice,model_output$prediction)
#gives value of 0.418718.

#rmse(test$SalePrice, prediction)
#0.0326 vs 0.02677 for random forest...RF better..use.
...
```
2.5 What kind of house will be sold for higher price?  - correlation shows variables of that type house
...
Make difference if remove foundaionPcon variable?? - top 7 features model??
```{r}

top7set <- new_95percentile[, c("SalePrice", "Total.Sq.Feet", "Overall.Qual", "Total.Bathrooms", "Garage.Cars", "Bsmt.Qual", "Kitchen.Qual", "Year.Remod.Add")]

top7set_1 <- sample(nrow(top7set), floor(nrow(top7set)*0.7))
train_new_7 <- top7set[top7set_1,]

test_new_7 <- top7set[-top7set_1,]

third_model <- lm(SalePrice ~ ., data = train_new_7)

prediction_new_7 <- predict(third_model, test_new_7, type = 'response')
model_output_new_7 <- cbind(test_new_7, prediction_new_7)

rmse(test_new_7$SalePrice, prediction_new_7)

summary(third_model)
#rmse is a bit better at 0.316 vs 0.343.
```
...
```
Evaluate Model Performance //Crossvalidation in R 10-fold try look at error measures
```{r}
 #10 fold cross validation try
...
#Need to install to use cv.lm()
#install.packages("DAAG")
library(DAAG)


cv.lm(data=top8set, form.lm=second_model, m= 10, plotit = F)
#10 folds of 212 observations
#sum of squares, mean sum of squares and f ratio
#anova table for results. sum of squares divided by degree of freedom equals mean of squares
#A large F ratio means that the variation among group means is more than you'd expect to see by chance

#USE IN REPORT BELOW
cv.lm(data=top8set, form.lm=second_model, m= 10, dots = FALSE, seed=33, plotit=TRUE, printit=TRUE)
#provides visual to use in report!!
#mean square error is 0.00084 so really low. Overall ms is similar to the ms of each k-fold, does that suggest that the predictive accuracy is good?...most round out to be 0 so would save very similiar in each fold. 

#Lesser the MSE => smaller is the error => better the estimator.
#mse 0.00084 really good

#HAs anova included so 
#Null hypothesis: the means of the different groups are the same
#Alternative hypothesis: At least one sample mean is not equal to the others.
#since p value is below 0.05 can conclude statistical significance between groups..all but foundationPconc. 
````


second model predict on test set new
```{r}
...
rmse(test_new$SalePrice,prediction_new)
...
```{r}
shapiro.test(second_model_predict$Residual) #test normality of residuals.
#he null-hypothesis of this test is that the population is normally distributed. Thus, on the one hand, if the p-value is less than the chosen alpha level, then the null hypothesis is rejected and there is evidence that the data tested are not normally distributed.
#alpha level of 0.05 95% accuarte...p value is below of can accept null hypothsus as normally distributed significant. residuals.

t.test(top8set$Total.Sq.Feet, top8set$Overall.Qual)
#unpaired two sample t-test used to compare mean of two independent groups
...
t.test(top8set$Garage.Cars, top8set$Bsmt.Qual)
#p value below 0.05 so means are significantly different from eachother keep

t.test(top8set$FoundationPConc, top8set$Total.Sq.Feet)
#all significant different means. 
...
print(mean(abs(residuals_new) < (rmse(submission$Actual, submission$Predicted))))
#observations within 1 rmse
#0.7386 proportion of values 
print(mean(abs(residuals_new) < 2*(rmse(submission$Actual, submission$Predicted))))
#within 2 rmse 0.945%..so 95 percent of observations within 2 rmse! pretty good

#GOOD POINT FOR REPORT...rmse looks good for predictions
...
Attempts OTHER MODELS/
 have set up! - RandomForest tried in kaggle since R version didnt support package.
```{r}

#result
#randomforest rmse is 0.02677****** extremely low.  range of data is 0 to 1 though because normalized..even response variable.
#https://www.kaggle.com/alexanderemeny/kernel8ca48731d8/edit# link to kaggle working on randomforest!!

#rmse(test_new_7$SalePrice, prediction7)

#test crossvalidation on r
#rf.crossValidation(object, xdata, p = 0.1, n = 99, seed = NULL, normalize = FALSE, bootstrap = FALSE, trace = FALSE, ...)

#install.packages('rfUtilities')
#library(rfUtilities)
#rf.crossValidation(RF_model, data, p = 0.1, n = 99, seed = NULL, normalize = FALSE, bootstrap = FALSE, trace = FALSE, ...)


#data <- read.csv("../input/top7features/top7set.csv") #write this line in kaggle to retreive, found path by clicking side visual of dataframe and copying path

#data_7 <- sample(nrow(data), floor(nrow(data)*0.7))
#train_77 <- data[data_7,]
...
#RF_model_77 <- randomForest(SalePrice~., data=train_77)
#install.packages('randomForest')
#library(randomForest)
...
#Forest
#random_forest <- randomForest(SalePrice ~., data = train_new_7, importance= TRUE, ntree=500, na.action = na.roughfix)
#rfp <- predict(random_forest, test_new_7, type = 'response')
#rmse(data$SalePrice, rfp)
```
top7set <- new_95percentile[, c("SalePrice", "Total.Sq.Feet", "Overall.Qual", "Total.Bathrooms", "Garage.Cars", "Bsmt.Qual", "Kitchen.Qual", "Year.Remod.Add")]

#write.csv(top7set, file = "~/Desktop/top7set.csv") #way to save dataframe to computer. 
data <- read.csv("../input/top7features/top7set.csv") #write this line in kaggle to retreive, found path by clicking side visual of dataframe and copying path
...
prediction_new_7 <- predict(RF_model, test_new_7, type = 'response')
model_output_new_7 <- cbind(test_new_7, prediction_new_7)

rmse(top7set$SalePrice,prediction_new_7)
...
#Overfitting is suspect when the model accuracy is high with respect to the data used in training the model but drops significantly with new data.
#rmse on test set vs rmse on all data. is there drop off?
...
OUTLINE FOR AREAS IM LOOKING AT FOR OWN ORGANIZATION.
----------------------------------------------------------------------------------
